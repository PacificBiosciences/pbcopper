...
MovieName::MovieName(const std::string& runStartTime,
...
void MovieName::UpdatePartsCache(void) const
{
    ...
    const auto movieCStr = movieName_.c_str();
    ...
    if (partsCache_->serialNumber_.empty())
        throw runtime_error("MovieName: instrument serial number must not be empty");
    if (partsCache_->smrtCellBarcode_.empty())
        throw runtime_error("MovieName: SMRT cell barcode must not be empty");
    if (partsCache_->setNumber_.empty())
        throw runtime_error("MovieName: set number must not be empty");
    if (partsCache_->partNumber_.empty())
        throw runtime_error("MovieName: part number must not be empty");
 }
