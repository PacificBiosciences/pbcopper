...
char CigarOperation::TypeToChar(const CigarOperationType type)
{
    ...
    return LOOKUP_TABLE[static_cast<std::int32_t>(type)];
}

bool ConsumesQuery(const CigarOperationType type) noexcept
{
    //                                X=PHSNDIM
    constexpr std::uint32_t LOOKUP_TABLE{0b110010011};
    const auto val = static_cast<std::uint32_t>(type);
    assert(val <= 8);
...
{
    //                                X=PHSNDIM
    constexpr std::uint32_t LOOKUP_TABLE{0b110001101};
    const auto val = static_cast<std::uint32_t>(type);
    assert(val <= 8);
