...
GlobalLocalResult GlobalLocalLastRowMax(const std::vector<std::int32_t>& lastRow) noexcept
{
    const auto maxElementIt = std::max_element(std::cbegin(lastRow), std::cend(lastRow));
    const std::int32_t maxElementPos = maxElementIt - std::cbegin(lastRow);
...
GlobalLocalResult GlobalLocalAlign(const char* const query, const std::int32_t queryLength,
                                   const char* const read, const std::int32_t readLength,
                                   const GlobalLocalParameters& parameters) noexcept
{
    GlobalLocalStorage storage{};
                                   ...
                                   GlobalLocalStorage& storage) noexcept
{
    ...
    const std::int32_t m = queryLength + 1;
    const std::int32_t n = readLength + 1;

    std::vector<std::int32_t> lastRow(readLength);
    ...
    std::int32_t prevOffset = 0;
    std::int32_t curOffset = m;
    ...
    const std::int32_t mismatchDelta = parameters.MatchScore - parameters.MismatchPenalty;
    const std::int32_t insertionDelta = parameters.BranchPenalty - parameters.InsertionPenalty;
    const std::int32_t deletionDelta = parameters.MergePenalty - parameters.DeletionPenalty;
    ...
    for (std::int32_t j = 1; j < n; ++j) {  // rows
        std::swap(prevOffset, curOffset);
        const char prevRead = read[j - 1];
        const std::int32_t readMismatch = (prevRead != read[j]);
        for (std::int32_t i = 1; i < m; ++i) {  // cols

            // Match/mismatch
            const std::int32_t a{col[i - 1 + prevOffset] + parameters.MatchScore -
                                 (mismatchDelta * (prevRead != query[i - 1]))};

            // Insertion in the read or branch if the current and last read sequence are identical
            const std::int32_t b{col[i + prevOffset] + parameters.BranchPenalty -
                                 (insertionDelta * readMismatch)};

            // Deletion in the read or merge if the current and last read sequence are identical
            const std::int32_t c{col[i - 1 + curOffset] + parameters.MergePenalty -
