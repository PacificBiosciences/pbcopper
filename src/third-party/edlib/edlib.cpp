...
public:
    EqualityDefinition(const string& alphabet, const EdlibEqualityPair* additionalEqualities = NULL,
...
static int myersCalcEditDistanceSemiGlobal(const Word* Peq, int W, int maxNumBlocks,
                                           ...
                                           int targetLength, int k, EdlibAlignMode mode,
...
static int myersCalcEditDistanceNW(const Word* Peq, int W, int maxNumBlocks, int queryLength,
                                   const unsigned char* target, int targetLength, int k,
                                   int* bestScore_, int* position_, bool findAlignment,
...
static int obtainAlignment(const unsigned char* query, const unsigned char* rQuery, int queryLength,
                           const unsigned char* target, const unsigned char* rTarget,
                           int targetLength, const EqualityDefinition& equalityDefinition,
...
static int obtainAlignmentTraceback(int queryLength, int targetLength, int bestScore,
                                    const AlignmentData* alignData, unsigned char** alignment,
                                    int* alignmentLength);
                                       ...
                                       const EdlibAlignConfig config)
{
    ...
    int W =
        maxNumBlocks * WORD_SIZE - queryLength;  // number of redundant cells in last level blocks
    ...
    do {
        if (config.mode == EDLIB_MODE_HW || config.mode == EDLIB_MODE_SHW) {
            myersCalcEditDistanceSemiGlobal(Peq, W, maxNumBlocks, queryLength, target, targetLength,
    ...
    if (result.editDistance >= 0) {  // If there is solution.
        ...
        if (config.task == EDLIB_TASK_LOC || config.task == EDLIB_TASK_PATH) {
            ...
            if (config.mode == EDLIB_MODE_HW) {  // If HW, I need to calculate start locations.
                ...
                Word* rPeq = buildPeq(static_cast<int>(alphabet.size()), rQuery, queryLength,
                                      equalityDefinition);
                for (int i = 0; i < result.numLocations; i++) {
                    ...
                    if (endLocation == -1) {
                        ...
                        myersCalcEditDistanceSemiGlobal(rPeq, W, maxNumBlocks, queryLength,
                                                        ...
                                                        endLocation + 1, result.editDistance,
                                                        EDLIB_MODE_SHW, &bestScoreSHW,
        ...
        if (config.task == EDLIB_TASK_PATH) {
            ...
            obtainAlignment(query, rQuery, queryLength, alnTarget, rAlnTarget, alnTargetLength,
                            equalityDefinition, static_cast<int>(alphabet.size()),
 ...
 */
static inline int calculateBlock(Word Pv, Word Mv, Word Eq, const int hin, Word& PvOut, Word& MvOut)
{
    // hin can be 1, -1 or 0.
    // 1  -> 00...01
    // 0  -> 00...00
 ...
 * @return Status.
 */
static int myersCalcEditDistanceSemiGlobal(const Word* const Peq, const int W,
                                           ...
                                           int* const bestScore_, int** const positions_,
                                           int* const numPositions_)
{
    *positions_ = NULL;
    *numPositions_ = 0;
    ...
    const int startHout =
        mode == EDLIB_MODE_HW ? 0 : 1;  // If 0 then gap before query is not penalized;
    ...
    for (int c = 0; c < targetLength; c++) {  // for each column
        ...
        if (lastBlock < firstBlock) {
            *bestScore_ = bestScore;
            if (bestScore != -1) {
                *positions_ =
                    static_cast<int*>(malloc(sizeof(int) * static_cast<int>(positions.size())));
        ...
        if (lastBlock == maxNumBlocks - 1) {
            ...
            if (colScore <=
                k) {  // Scores > k dont have correct values (so we cannot use them), but are certainly > k.
...
static int myersCalcEditDistanceNW(const Word* const Peq, const int W, const int maxNumBlocks,
                                   ...
                                   const int targetLength, int k, int* const bestScore_,
                                   ...
                                   AlignmentData** const alignData, const int targetStopPosition)
{
    ...
    const int STRONG_REDUCE_NUM =
        2048;  // TODO: Choose this number dinamically (based on query and target lengths?), so it does not affect speed of computation
    ...
    int lastBlock =
        min(maxNumBlocks, ceilDiv(min(k, (k + queryLength - targetLength) / 2) + 1, WORD_SIZE)) - 1;
    Block* bl;  // Current block
                           ...
                           int* const alignmentLength)
{
    ...
    if (alignmentDataSize < 1024 * 1024) {
        ...
        statusCode = obtainAlignmentHirschberg(query, rQuery, queryLength, target, rTarget,
                                               targetLength, equalityDefinition, alphabetLength,
                                     ...
                                     unsigned char** const alignment, int* const alignmentLength)
{
    ...
    int leftHalfCalcStatus = myersCalcEditDistanceNW(
        Peq, W, maxNumBlocks, queryLength, target, targetLength, bestScore, &score_, &endLocation_,
        false, &alignDataLeftHalf, leftHalfWidth - 1);

    // Calculate right half.
    AlignmentData* alignDataRightHalf = NULL;
    int rightHalfCalcStatus = myersCalcEditDistanceNW(
        rPeq, W, maxNumBlocks, queryLength, rTarget, targetLength, bestScore, &score_,
        &endLocation_, false, &alignDataRightHalf, rightHalfWidth - 1);
    ...
    int queryIdxLeftEnd =
        min(scoresLeftStartIdx + scoresLeftLength - 1, scoresRightStartIdx + scoresRightLength - 2);
    ...
    int ulStatusCode = obtainAlignment(query, rQuery + lrHeight, ulHeight, target,
                                       rTarget + lrWidth, ulWidth, equalityDefinition,
