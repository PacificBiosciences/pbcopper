                     ...
                     int zdrop, int end_bonus, int flag, ksw_extz_t* ez)
{
    ...
    mem = (uint8_t*)pbkcalloc(km, tlen_ * 8 + qlen_ + 1, 16);
    ...
    if (!approx_max) {
        H = (int32_t*)pbkmalloc(km, tlen_ * 16 * 4);
        for (t = 0; t < tlen_ * 16; ++t)
            H[t] = KSW_NEG_INF;
    }
    if (with_cigar) {
        mem2 = (uint8_t*)pbkmalloc(km, ((size_t)(qlen + tlen - 1) * n_col_ + 1) * 16);
        p = (simde__m128i*)(((size_t)mem2 + 15) >> 4 << 4);
        off = (int*)pbkmalloc(km, (qlen + tlen - 1) * sizeof(int) * 2);
    ...
    for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r) {
        ...
        if (!(flag & KSW_EZ_GENERIC_SC)) {
            for (t = st0; t <= en0; t += 16) {
                ...
                mask =
                    simde_mm_or_si128(simde_mm_cmpeq_epi8(sq, m1_), simde_mm_cmpeq_epi8(st2, m1_));
                tmp = simde_mm_cmpeq_epi8(sq, st2);
        ...
        if (!with_cigar) {  // score only
            for (t = st_; t <= en_; ++t) {
                ...
                simde_mm_store_si128(
                    &u[t], simde_mm_sub_epi8(z, vt1)); /* u[r][t..t+15] <- z - v[r-1][t-1..t+14] */
                simde_mm_store_si128(
                    &v[t], simde_mm_sub_epi8(z, ut)); /* v[r][t..t+15] <- z - u[r-1][t..t+15] */
            ...
            for (t = st_; t <= en_; ++t) {
                ...
                b2 = simde_mm_add_epi8(simde_mm_load_si128(&y2[t]), ut);
                /////////////////////////////////////
                d = simde_mm_and_si128(simde_mm_cmpgt_epi8(a, z),
                                       simde_mm_set1_epi8(1));  // d = a  > z? 1 : 0
                z = simde_mm_max_epi8(z, a);
                d = simde_mm_blendv_epi8(d, simde_mm_set1_epi8(2),
                                         simde_mm_cmpgt_epi8(b, z));  // d = b  > z? 2 : d
                z = simde_mm_max_epi8(z, b);
                d = simde_mm_blendv_epi8(d, simde_mm_set1_epi8(3),
                                         simde_mm_cmpgt_epi8(a2, z));  // d = a2 > z? 3 : d
                z = simde_mm_max_epi8(z, a2);
                d = simde_mm_blendv_epi8(d, simde_mm_set1_epi8(4),
                                         simde_mm_cmpgt_epi8(b2, z));  // d = a2 > z? 3 : d
                z = simde_mm_max_epi8(z, b2);
                z = simde_mm_min_epi8(z, sc_mch_);
            ...
            for (t = st_; t <= en_; ++t) {
                ...
                b2 = simde_mm_add_epi8(simde_mm_load_si128(&y2[t]), ut);
                /////////////////////////////////////
                d = simde_mm_andnot_si128(simde_mm_cmpgt_epi8(z, a),
                                          simde_mm_set1_epi8(1));  // d = z > a?  0 : 1
                z = simde_mm_max_epi8(z, a);
                d = simde_mm_blendv_epi8(simde_mm_set1_epi8(2), d,
                                         simde_mm_cmpgt_epi8(z, b));  // d = z > b?  d : 2
                z = simde_mm_max_epi8(z, b);
                d = simde_mm_blendv_epi8(simde_mm_set1_epi8(3), d,
                                         simde_mm_cmpgt_epi8(z, a2));  // d = z > a2? d : 3
                z = simde_mm_max_epi8(z, a2);
                d = simde_mm_blendv_epi8(simde_mm_set1_epi8(4), d,
                                         simde_mm_cmpgt_epi8(z, b2));  // d = z > b2? d : 4
                z = simde_mm_max_epi8(z, b2);
                z = simde_mm_min_epi8(z, sc_mch_);
                ...
                d = simde_mm_or_si128(
                    d,
                    simde_mm_andnot_si128(tmp, simde_mm_set1_epi8(0x08)));  // d = a > 0? 1<<3 : 0
                tmp = simde_mm_cmpgt_epi8(zero_, b);
                simde_mm_store_si128(&y[t], simde_mm_sub_epi8(simde_mm_andnot_si128(tmp, b), qe_));
                d = simde_mm_or_si128(
                    d,
                    simde_mm_andnot_si128(tmp, simde_mm_set1_epi8(0x10)));  // d = b > 0? 1<<4 : 0
                tmp = simde_mm_cmpgt_epi8(zero_, a2);
                simde_mm_store_si128(&x2[t],
                                     simde_mm_sub_epi8(simde_mm_andnot_si128(tmp, a2), qe2_));
                d = simde_mm_or_si128(
                    d,
                    simde_mm_andnot_si128(tmp, simde_mm_set1_epi8(0x20)));  // d = a > 0? 1<<5 : 0
                tmp = simde_mm_cmpgt_epi8(zero_, b2);
                simde_mm_store_si128(&y2[t],
                                     simde_mm_sub_epi8(simde_mm_andnot_si128(tmp, b2), qe2_));
                d = simde_mm_or_si128(
                    d,
                    simde_mm_andnot_si128(tmp, simde_mm_set1_epi8(0x40)));  // d = b > 0? 1<<6 : 0
                simde_mm_store_si128(&pr[t], d);
            }
        }
        if (!approx_max) {  // find the exact max with a 32-bit score array
            ...
            if (r > 0) {
                ...
                for (t = st0; t < en1;
                     t +=
                     4) {  // this implements: H[t]+=v8[t]-qe; if(H[t]>max_H) max_H=H[t],max_t=t;
                ...
                for (; t < en0;
                     ++t) {  // for the rest of values that haven't been computed with SSE
                    H[t] += (int32_t)v8[t];
            ...
            // update ez
            if (en0 == tlen - 1 && H[en0] > ez->mte) ez->mte = H[en0], ez->mte_q = r - en;
            if (r - st0 == qlen - 1 && H[st0] > ez->mqe) ez->mqe = H[st0], ez->mqe_t = st0;
            if (ksw_apply_zdrop(ez, 1, max_H, r, max_t, zdrop, e2)) break;
            if (r == qlen + tlen - 2 && en0 == tlen - 1) ez->score = H[tlen - 1];
    ...
    pbkfree(km, mem);
    if (!approx_max) pbkfree(km, H);
    if (with_cigar) {  // backtrack
        int rev_cigar = !!(flag & KSW_EZ_REV_CIGAR);
        if (!ez->zdropped && !(flag & KSW_EZ_EXTZ_ONLY)) {
            ksw_backtrack(km, 1, rev_cigar, 0, (uint8_t*)p, off, off_end, n_col_ * 16, tlen - 1,
                          qlen - 1, &ez->m_cigar, &ez->n_cigar, &ez->cigar);
        } else if (!ez->zdropped && (flag & KSW_EZ_EXTZ_ONLY) &&
                   ez->mqe + end_bonus > (int)ez->max) {
            ez->reach_end = 1;
            ksw_backtrack(km, 1, rev_cigar, 0, (uint8_t*)p, off, off_end, n_col_ * 16, ez->mqe_t,
                          qlen - 1, &ez->m_cigar, &ez->n_cigar, &ez->cigar);
        } else if (ez->max_t >= 0 && ez->max_q >= 0) {
            ksw_backtrack(km, 1, rev_cigar, 0, (uint8_t*)p, off, off_end, n_col_ * 16, ez->max_t,
                          ez->max_q, &ez->m_cigar, &ez->n_cigar, &ez->cigar);
        }
        pbkfree(km, mem2);
        pbkfree(km, off);
    }
                     ...
                     int end_bonus, int flag, ksw_extz_t* ez)
{
    ...
    int32_t *H = 0, H0 = 0, last_H0_t = 0;
    uint8_t *qr, *sf, *mem, *mem2 = 0;
    simde__m128i q_, qe2_, zero_, flag1_, flag2_, flag8_, flag16_, sc_mch_, sc_mis_, sc_N_, m1_,
        max_sc_;
    ...
    mem = (uint8_t*)pbkcalloc(km, tlen_ * 6 + qlen_ + 1, 16);
    u = (simde__m128i*)(((size_t)mem + 15) >> 4 << 4);  // 16-byte aligned
    v = u + tlen_, x = v + tlen_, y = x + tlen_, s = y + tlen_, sf = (uint8_t*)(s + tlen_),
    qr = sf + tlen_ * 16;
    if (!approx_max) {
        H = (int32_t*)pbkmalloc(km, tlen_ * 16 * 4);
        for (t = 0; t < tlen_ * 16; ++t)
    ...
    for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r) {
        ...
        if (!with_cigar) {  // score only
            ...
            for (t = st_; t <= en_; ++t) {
                ...
                d = simde_mm_or_si128(d,
                                      simde_mm_andnot_si128(tmp, flag8_));  // d = 0 > a? 0 : 0x08
                tmp = simde_mm_cmpgt_epi8(zero_, b);
                simde_mm_store_si128(&y[t], simde_mm_andnot_si128(tmp, b));
                d = simde_mm_or_si128(d,
                                      simde_mm_andnot_si128(tmp, flag16_));  // d = 0 > b? 0 : 0x10
                simde_mm_store_si128(&pr[t], d);
            }
        }
        if (!approx_max) {  // find the exact max with a 32-bit score array
...
void ksw_exts2_simde(void* km, int qlen, const uint8_t* query, int tlen, const uint8_t* target,
                     int8_t m, const int8_t* mat, int8_t q, int8_t e, int8_t q2, int8_t noncan,
                     int zdrop, int flag, ksw_extz_t* ez)
{
    int r, t, qe = q + e, n_col_, *off = 0, *off_end = 0, tlen_, qlen_, last_st, last_en, max_sc,
              min_sc, long_thres, long_diff;
    int with_cigar = !(flag & KSW_EZ_SCORE_ONLY), approx_max = !!(flag & KSW_EZ_APPROX_MAX);
    int32_t *H = 0, H0 = 0, last_H0_t = 0;
    uint8_t *qr, *sf, *mem, *mem2 = 0;
    ...
    long_thres = (q2 - q) / e - 1;
    if (q2 > q + e + long_thres * e) ++long_thres;
    long_diff = long_thres * e - (q2 - q);

    mem = (uint8_t*)pbkcalloc(km, tlen_ * 9 + qlen_ + 1, 16);
    u = (simde__m128i*)(((size_t)mem + 15) >> 4 << 4);  // 16-byte aligned
    ...
    for (r = 0, last_st = last_en = -1; r < qlen + tlen - 1; ++r) {
        ...
        if (!with_cigar) {  // score only
            for (t = st_; t <= en_; ++t) {
                ...
                // __pancake_dp_code_block2;  // save u[] and v[]; update a, b and a2
                /////////////////////////////////////
                ///// __pancake_dp_code_block2; /////
                simde_mm_store_si128(
                    &u[t], simde_mm_sub_epi8(z, vt1)); /* u[r][t..t+15] <- z - v[r-1][t-1..t+14] */
            ...
            for (t = st_; t <= en_; ++t) {
                ...
                z = simde_mm_max_epi8(z, b);
                d = simde_mm_blendv_epi8(d, simde_mm_set1_epi8(3),
                                         simde_mm_cmpgt_epi8(a2a, z));  // d = a2 > z? 3 : d
                z = simde_mm_max_epi8(z, a2a);
                /////////////////////////////////////
                ///// __pancake_dp_code_block2; /////
                simde_mm_store_si128(
            ...
            for (t = st_; t <= en_; ++t) {
                ...
                z = simde_mm_max_epi8(z, b);
                d = simde_mm_blendv_epi8(simde_mm_set1_epi8(3), d,
                                         simde_mm_cmpgt_epi8(z, a2a));  // d = z > a2? d : 3
                z = simde_mm_max_epi8(z, a2a);
                /////////////////////////////////////
                ///// __pancake_dp_code_block2; /////
                simde_mm_store_si128(
        ...
        if (!approx_max) {  // find the exact max with a 32-bit score array
            ...
            if (r > 0) {
                ...
                H0 = v8[0] - qe, last_H0_t = 0;
            if ((flag & KSW_EZ_APPROX_DROP) && ksw_apply_zdrop(ez, 1, H0, r, last_H0_t, zdrop, 0))
                break;
            if (r == qlen + tlen - 2 && en0 == tlen - 1) ez->score = H0;
        }
        last_st = st, last_en = en;
    ...
    if (with_cigar) {  // backtrack
        int rev_cigar = !!(flag & KSW_EZ_REV_CIGAR);
        if (!ez->zdropped && !(flag & KSW_EZ_EXTZ_ONLY))
            ksw_backtrack(km, 1, rev_cigar, long_thres, (uint8_t*)p, off, off_end, n_col_ * 16,
