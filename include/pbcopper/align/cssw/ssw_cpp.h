...
struct Alignment
{
    std::uint16_t sw_score;            // The best alignment score
    std::uint16_t sw_score_next_best;  // The next best alignment score
    std::int32_t ref_begin;            // Reference begin position of the best alignment
    std::int32_t ref_end;              // Reference end position of the best alignment
    std::int32_t query_begin;          // Query begin position of the best alignment
    std::int32_t query_end;            // Query end position of the best alignment
    std::int32_t ref_end_next_best;    // Reference end position of the next best alignment
    std::int32_t mismatches;           // Number of mismatches of the alignment
    std::string cigar_string;          // Cigar string of the best alignment
    std::vector<std::uint32_t> cigar;  // Cigar stored in the BAM format
                                       //   high 28 bits: length
...
struct Filter
{
    ...
    std::uint16_t score_filter;     // score >= score_filter
    std::uint16_t distance_filter;  // ((ref_end - ref_begin) < distance_filter) &&
    ...
    Filter(const bool& pos, const bool& cigar, const std::uint16_t& score, const std::uint16_t& dis)
...
public:
    ...
    //             use the other constructor and pass the corresponding matrix in.
    // =========
    Aligner(const std::uint8_t& match_score, const std::uint8_t& mismatch_penalty,
    ...
    // =========
    Aligner(const std::int8_t* score_matrix, const int& score_matrix_size,
    ...
    // =========
    void SetGapPenalty(const std::uint8_t& opening, const std::uint8_t& extending)
    {
    ...
    // @return   True: succeed; false: fail.
    // =========
    bool ReBuild(const std::uint8_t& match_score, const std::uint8_t& mismatch_penalty,
    ...
    // @return   True: succeed; false: fail.
    // =========
    bool ReBuild(const std::int8_t* score_matrix, const int& score_matrix_size,
                 const std::int8_t* translation_matrix, const int& translation_matrix_size);

private:
    std::int8_t* score_matrix_;
    int score_matrix_size_;
    std::int8_t* translation_matrix_;

    std::uint8_t match_score_;            // default: 2
    std::uint8_t mismatch_penalty_;       // default: 2
    std::uint8_t gap_opening_penalty_;    // default: 3
    std::uint8_t gap_extending_penalty_;  // default: 1

    std::int8_t* translated_reference_;
    std::int32_t reference_length_;

    int TranslateBase(const char* bases, const int& length, std::int8_t* translated) const;
